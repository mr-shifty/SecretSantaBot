...
async def ensure_sample_entries():
    ...
    async with async_session as session:
        ...
        e1 = Route1Entry(user_id=u1.id, email='t1@example.com', wishlist='Люблю книги', status='completed', pickup_type='postal', postal_city='CityA', postal_street='Main', postal_building='1', postal_index='123456', postal_recipient_last_name='Иванов', postal_recipient_first_name='Иван', postal_recipient_patronymic='Иванович', full_address='123456, CityA, Main д. 1')
        e2 = Route1Entry(user_id=u2.id, email='t2@example.com', wishlist='Люблю чай', status='completed', pickup_type='pickup', pickup_company='СДЭК', pickup_address='Пункт CDEK №125', postal_index='141488', pickup_recipient_fullname='Петров Пётр', pickup_recipient_phone='8800555353', full_address='СДЭК, индекс 141488, Пункт CDEK №125')
...
async def preview_notifications(route=1):
    ...
    # Use shared helper to build preview message per giver
    previews = []
    async_session = get_session()
    async with async_session as session:
