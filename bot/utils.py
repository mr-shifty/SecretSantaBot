...
async def save_route1_entry(user_id: int, email: str, full_address: str, delivery_method: str | None, wishlist: str | None, phone: str | None = None):
	async_session = get_session()
	async with async_session as session:
		entry = Route1Entry(
			user_id=user_id,
		...
		logger.info(f"Route1 entry created for user {user_id}: {email}")
...
async def save_route2_entry(user_id: int, email: str, phone: str | None = None):
	async_session = get_session()
	async with async_session as session:
		entry = Route2Entry(
			user_id=user_id,
